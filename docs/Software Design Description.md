...
| 12  | Filter Dashboard Search Results        | Filter changes trigger refetch (or client filtering) and update counts                  |
...
sequenceDiagram
    ...
    App->>ElectionAPI: GET /auth/me (or /session)
...
sequenceDiagram
    ...
    RequireAuth->>ElectionAPI: GET /auth/me (or /session)
...
sequenceDiagram
    ...
    User->>MapPage: Select constituency (dropdown or map click)
    MapPage->>MapPage: Set selectedConstituency
    MapPage->>MapView: focusTarget updated (geometry/bounds + sidebarInsetPx)

    MapView->>MapView: Compute bounds (from geometry or bbox)
...
**Explanation:**
Filter changes trigger a new search query (or re-filtering) to keep the table consistent with analysis intent.
